...
Public Type AutoSave
    ...
    IsTimestampSavePending As Boolean
    TimestampSaveDelay As Integer
End Type
...
Public Sub DoAutoSave(TmrDatabase As ProjectTimers, Optional Forced As Boolean = False)
    ...
    'Second component of the save operation is happening now
    If (PeriodicSave.IsTimestampSavePending = True And SecSinceLastSave >= PeriodicSave.TimestampSaveDelay) Then
        Call DoSaveAllToUserDir(TmrDatabase)
        PeriodicSave.IsTimestampSavePending = False
    End If
    ...
    If ((SecSinceLastSave >= PeriodicSave.SaveEvery And PeriodicSave.AreTimersChanged = True) Or Forced = True) Then
        ...
        'Delay the second save call (in case of a windows crash during the time of saving)
        PeriodicSave.IsTimestampSavePending = True
...
Public Sub DoStoreSettings(TmrDatabase As ProjectTimers)
    ...
    Dim ProbeResult As Variant
    Dim SavePath As String
    
    SavePath = TimersIOPath
    ...
    ProbeResult = ProbeThisPath(SavePath)
    If (Not IsArray(ProbeResult)) Then Exit Sub
    
    If (ProbeResult(4) = vbFalse) Then
        SavePath = GetShellFolderPath(CSIDL_DESKTOP) & "\ProjectTimers-Rescued.ini"
        ProbeResult = ProbeThisPath(SavePath)
        If (ProbeResult(4) = vbFalse) Then Exit Sub
    End If
...
Public Sub DoSaveOneTimer(TmrDatabase As ProjectTimers, ThisTimer As Long, Optional SurpressConfDlg As Boolean = True, Optional SurpressErrDlg As Boolean = False)
    Dim OneFileLine As String
    Dim FileNum As Integer
    ...
    ProbeResult = ProbeThisPath(OnePath)
    If (Not IsArray(ProbeResult)) Then Exit Sub
    
    If (ProbeResult(4) = vbTrue) Then
        OkToSave = True
    Else
        If (SurpressErrDlg = False) Then UsrDlgResp = MsgBox("Should this timestamp be saved anyway ?" & vbCrLf & "Timestamp for timer <" & TmrDatabase.MyTimers(ThisTimer).Text & "> is not yet saved because:" & vbCrLf & vbCrLf & ProbeResult(3), vbOKCancel + vbDefaultButton2, "Caution while saving this timestamp") Else UsrDlgResp = vbCancel
        If (UsrDlgResp = vbCancel) Then OkToSave = False Else OkToSave = True
    ...
    If (OkToSave = True) Then
        ...
        If (Err.Number = 0 And SurpressConfDlg = False) Then MsgBox "Timer <" & TmrDatabase.MyTimers(ThisTimer).Text & "> has been saved", vbOKOnly, "Complete"
        If (Err.Number <> 0 And SurpressErrDlg = False) Then MsgBox "An attempt was made to save the timestamp for timer <" & TmrDatabase.MyTimers(ThisTimer).Text & "> " & vbCrLf & "However, Windows reported the following error while trying to handle the file:" & vbCrLf & "Err " & Err.Number & ": " & Err.Description & vbCrLf & vbCrLf & "Possibly because:" & vbCrLf & ProbeResult(3), vbOKOnly, "Error report"
    
    End If
    Err.Clear
End Sub

'Receives a path and checks if it is accessible for writing
Private Function ProbeThisPath(ThisPath As String) As Variant
    Dim OnePath As String
    Dim ProbeResult(4) As Variant
    Dim ToCheck As String
    Dim OneAttr As Integer
    Dim Directories As Variant
    Dim ErrReport As String
    ...
    If (UBound(Directories) < 1) Then Exit Function
    ...
    If (Err.Number = 53) Then OneAttr = -1: Err.Description = ""
    ...
    If (Err.Number = 76) Then OneAttr = -2: Err.Description = ""
    ProbeResult(1) = Array(OneAttr, GetAttrDescription(OneAttr), Err.Number, Err.Description, OnePath)
    Err.Clear 'error code 76 is for path not found (53 is for file not found)
    
    'Check FilePermissions
    ProbeResult(2) = CheckPermissions(OnePath)
    
    ErrReport = "Check1 = Attr(" & ProbeResult(0)(0) & " " & ProbeResult(0)(1) & "), Err(" & ProbeResult(0)(2) & " " & ProbeResult(0)(3) & "). Dir =" & ProbeResult(0)(4) & vbCrLf
    ErrReport = ErrReport & "Check2 = Attr(" & ProbeResult(1)(0) & " " & ProbeResult(1)(1) & "), Err(" & ProbeResult(1)(2) & " " & ProbeResult(1)(3) & ")" & vbCrLf
    ErrReport = ErrReport & "Check3 = File permissions for user <" & ProbeResult(2)(0) & "> Read:" & ProbeResult(2)(1) & " Write:" & ProbeResult(2)(2)
    
    ProbeResult(3) = ErrReport
    
    'Is it ok to save on this path ?
    If (ProbeResult(1)(2) = 53 Or (ProbeResult(2)(1) = vbTrue And ProbeResult(2)(2) = vbTrue And ProbeResult(0)(2) <> 76 And ProbeResult(0)(2) <> 53 And ProbeResult(1)(2) <> 76 And (CInt(ProbeResult(0)(0)) And (1 + 2 + 4 + 16 + 32 + 64 + 128 + 256 + 512 + 1024 + 2048 + 4096 + 8192 + 16384)) = CInt(ProbeResult(0)(0)) And (CInt(ProbeResult(1)(0)) And (16 + 32 + 64 + 128 + 256 + 512 + 1024 + 2048 + 4096 + 8192 + 16384)) = CInt(ProbeResult(1)(0)))) Then ProbeResult(4) = vbTrue Else ProbeResult(4) = vbFalse
    
    ProbeThisPath = ProbeResult
    Err.Clear
...
Public Function GetAttrDescription(ByVal AttrCode As Integer) As String
    ...
    If (AttrCode = 0) Then GetAttrDescription = VBAttrDescr(0): Exit Function
    If (AttrCode = -1) Then GetAttrDescription = "DirNotFound": Exit Function
    If (AttrCode = -2) Then GetAttrDescription = "FileNotFound": Exit Function
